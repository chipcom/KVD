...
METHOD Set( cSection, cEntry, uValue ) CLASS TIniFile

   local cType := ValType( uValue )
   
    DO CASE
		CASE cType == "N"
			::WriteNumber( cSection, cEntry, uValue )
		CASE cType == "C"
			::WriteString( cSection, cEntry, uValue )
		CASE cType == "D"
			::WriteDate( cSection, cEntry, uValue )
		CASE cType == "L"
			::WriteBool( cSection, cEntry, uValue )
	ENDCASE
	::UpdateFile()
...
METHOD Get( cSection, cEntry, uDefault, uVar ) CLASS TIniFile

   local cType := ValType( If( uDefault != nil, uDefault, uVar ) )

//   if Empty( ::cIniFile )
      if cType == "N"
		 uVar =	::ReadNumber( cSection, cEntry, uDefault )
//         uVar = GetProfInt( cSection, cEntry, uDefault )
      else
         uVar = ::ReadString( cSection, cEntry, cValToChar( uDefault ) )
...
//   DEFAULT cFileName := ""
   ...
   if ! Empty( cFileName ) .and. At( ".ini", LOWER(cFileName) ) == 0
      cFileName += ".ini"
   endif

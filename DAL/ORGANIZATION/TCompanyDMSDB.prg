...
// файл "p_d_smo.dbf"
CREATE CLASS TCompanyDMSDB	INHERIT	TBaseObjectDB
	VISIBLE:
		METHOD New()
		METHOD Save( oCompany )
		METHOD GetByID( nID )
		METHOD GetList()
...
METHOD New() CLASS TCompanyDMSDB
	return self

METHOD GetByID( nID )    CLASS TCompanyDMSDB
	local hArray := nil
	local ret := nil
...
METHOD GetList()    CLASS TCompanyDMSDB
	local aReturn := {}
	local oRow := nil
...
METHOD MenuCompanies( lReverse )		 CLASS TCompanyDMSDB
	local aCompany := {}
	local oRow := nil
...
METHOD Save( oCompany ) CLASS TCompanyDMSDB
	local ret := .f.
	local aHash := nil
	
	if upper( oCompany:classname() ) == upper( 'TCompanyDMS' )
		aHash := hb_Hash()
...
METHOD FillFromHash( hbArray )     CLASS TCompanyDMSDB
	local obj, oBank

	oBank := TBank():New( hbArray[ 'BANK' ], hbArray[ 'R_SCHET' ], hbArray[ 'K_SCHET' ], hbArray[ 'SMFO' ] )
	obj := TCompanyDMS():New( hbArray[ 'ID' ], ;
			hbArray[ 'NAME' ], ;
			hbArray[ 'FNAME' ], ;
			hbArray[ 'INN' ], ;
			hbArray[ 'ADRES' ], ;

...
CREATE CLASS T_okatosDB	INHERIT	TBaseObjectDB
	VISIBLE:
		METHOD New()
		METHOD getByID( nID )
...
METHOD New()		CLASS T_okatosDB
	return self

METHOD getListByOblast( param )		 CLASS T_okatosDB
...
METHOD getByID( nID )		 CLASS T_okatosDB
	local hArray := nil
	local ret := nil
...
METHOD function getByOKATO( param ) CLASS T_okatosDB
...
METHOD FillFromHash( hbArray )     CLASS T_okatosDB
	local obj
	
	obj := T_okatos():New( hbArray[ 'ID' ], ;
			hbArray[ 'REC_NEW' ], ;
			hbArray[ 'DELETED' ], ;
			)
